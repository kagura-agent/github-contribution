...
5. **症状 vs 根因：改值还是改控制流？** — claude-hud#462/#469 教训：看到 fallback/default 值不对时，不要直接改数字，先问「为什么代码会走到这个分支？」。看到输出不对时，不要先调格式，先问「这个分支是不是应该被跳过？」。正确的修复通常是增加条件判断（改控制流），而不是替换常量（改值）。维护者的方案往往代码量更多但更精准，因为他们明确了边界条件
